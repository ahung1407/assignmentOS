...
#ifdef IODUMP
  printf("alloc region=%d size=%d for process %d\n", reg_index, size, proc->pid);
#ifdef PAGETBL_DUMP
  ...
  if (PAGING_PAGE_PRESENT(pte)){
    if (!PAGING_PAGE_SWAPPED(pte)){
     ...
     // frame in SWAP, swap to RAM
      ...
      vicfpn = PAGING_FPN_RAM(caller -> mm -> pgd[vicpgn]);
...
int validate_overlap_vm_area(struct pcb_t *caller, int vmaid, int vmastart, int vmaend)
{
  ...
  // no need to implement cause there is only one VMA
  return 0;
}
...
int inc_vma_limit(struct pcb_t *caller, int vmaid, int inc_sz)
{
  ...
  int incnumpage =  (cur_vma->vm_end - old_end) / PAGING_PAGESZ;
